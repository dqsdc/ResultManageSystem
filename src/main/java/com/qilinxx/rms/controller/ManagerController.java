...
//        user.setState("2");//账号变为待审核状态
        ...
        if (selfFileList!=null&&selfFileList.size()!=0){
            ...
            for (MultipartFile file : selfFileList) {
                ...
                document.setPath("upload\\" + user.getUid() + "\\self\\"+ fileName);
                documentService.createDocument(document);
                //文件拷贝与删除
                File tempFile = new File(UploadUtil.getUploadFilePath() + "/upload//" + user.getUid() + "//temp//self", fileName);
                File targetFile = new File(UploadUtil.getUploadFilePath() + "/upload//" + user.getUid() + "//self//" , fileName);
                File pathFile = new File(UploadUtil.getUploadFilePath() + "/upload//" + user.getUid() + "//self//");
                pathFile.mkdirs();
                FileCopyUtils.copy(tempFile, targetFile);
                FileKit.deleteFile(tempFile);
            }
            FileKit.selfMap.remove(key);
            File dirFile = new File(UploadUtil.getUploadFilePath() + "/upload//" + user.getUid() + "//temp//self");
    ...
    public JSONObject ajaxSelfFile(String key, MultipartFile file, HttpSession session) throws IOException {
        ...
        if (i == 0) {
            ...
            String path = UploadUtil.getUploadFilePath() + "/upload//" + user.getUid() + "//temp//self";//存储的根路径 临时文件目录
